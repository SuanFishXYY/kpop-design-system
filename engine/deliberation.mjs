...
// v3.3.0 ? Mock 3-round Mixed Council deliberation. No LLM calls.
...
function statementFor(member, brief) {
  ...
  if (member.type === "group") {
    let voice = "";
    try { voice = synthesizeVoice(member.slug, { trait: specialty(member), brief }).split("\n").slice(0, 2).join(" "); }
    catch { voice = `${member.name} anchors ${specialty(member)}.`; }
